...
--- body ---
{/* 此页面由skill的SKILL.md通过website/scripts/generate-skill-docs.py自动生成。请编辑源文件SKILL.md，而非本页面。 */}

# 权重与偏差（Weights & Biases）

W&B：记录ML实验、超参数搜索、模型注册表、仪表板。
...
:::info
以下是Hermes在此技能触发时加载的完整技能定义。这是代理（Agent）在技能激活时看到的指令。
:::

# 权重与偏差（Weights & Biases）：ML实验追踪与MLOps

## 何时使用此技能

当您需要以下功能时，请使用权重与偏差（Weights & Biases，简称W&B）：
- **追踪ML实验**：自动记录指标
- **实时可视化训练**：通过实时仪表板
- **比较不同运行**：跨超参数和配置
- **优化超参数**：通过自动化超参数搜索（Sweeps）
- **管理模型注册表**：包含版本控制和溯源
- **协作ML项目**：通过团队工作区
- **追踪制品（Artifacts）**：管理数据集、模型、代码及其溯源
