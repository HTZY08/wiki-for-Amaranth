...
--- body ---
{/* 此页面由技能 SKILL.md 通过 website/scripts/generate-skill-docs.py 自动生成。请编辑源文件 SKILL.md，而非此页面。 */}
...
:::info
以下是 Hermes 在触发此技能时加载的完整技能定义。这是技能激活时代理（agent）看到的具体指令。
:::

# AudioCraft: 音频生成

使用 Meta 的 AudioCraft 进行文本转音乐和文本转音频生成（含 MusicGen、AudioGen 和 EnCodec）的全面指南。
...
- 创建音效和环境音频
- 构建音乐生成应用
- 需要旋律条件（melody-conditioned）音乐生成
- 需要立体声音频输出
- 需要可控的音乐生成并支持风格迁移

**主要特性：**
- **MusicGen**: 文本转音乐，支持旋律条件
- **AudioGen**: 文本转音效生成
- **EnCodec**: 高保真神经音频编解码器
- **多种模型规模**: 小（300M）到大（3.3B）
- **立体声支持**: 完整立体声音频生成
...
└────────────────────────┬─────────────────────────────────────┘
                         │
┌────────────────────────▼─────────────────────────────────────┐
│               Transformer 解码器（语言模型）                   │
│     自回归生成音频令牌（audio tokens）                         │
│     使用高效的令牌交叉模式                                     │
└────────────────────────┬─────────────────────────────────────┘
                         │
┌────────────────────────▼─────────────────────────────────────┐
...
| `musicgen-medium` | 1.5B | 文本转音乐 | 均衡型 |
...
| `top_k` | 250 | Top-k 采样 |
| `top_p` | 0.0 | 核采样（nucleus sampling，0 = 禁用） |
| `temperature` | 1.0 | 采样温度 |
| `cfg_coef` | 3.0 | 无分类器指导（classifier-free guidance）系数 |
...
model.set_generation_params(
    ...
    temperature=1.0,      # 创造性（越高变化越大）
    cfg_coef=3.0          # 文本跟随程度（越高越严格）
)
...
model.set_generation_params(
    duration=30,
    cfg_coef=3.0,
    cfg_coef_beta=None  # 禁用双重 CFG，以生成纯风格
)
...
demo = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(0.5, 2.0, value=1.0, label="温度（Temperature）"),
