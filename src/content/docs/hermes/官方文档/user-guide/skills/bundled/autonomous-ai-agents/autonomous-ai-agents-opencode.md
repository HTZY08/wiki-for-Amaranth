...
:::info
以下是 Hermes 在触发此技能时加载的完整技能定义。即该技能激活时代理所看到的指令。
:::

# OpenCode CLI

使用 [OpenCode](https://opencode.ai) 作为由 Hermes 终端/进程工具编排的自主编码工作器。OpenCode 是一个与提供商无关的开源 AI 编码代理（Agent），具备 TUI 和 CLI。

## 何时使用

- 用户明确要求使用 OpenCode
- 你需要一个外部编码代理（Agent）来实现/重构/审查代码
- 你需要长时间运行的编码会话并检查进度
...
不同的 Shell 环境可能解析到不同的 OpenCode 二进制文件。如果你的终端和 Hermes 间行为不一致，请检查：
...
```
terminal(command="opencode run '审查此配置文件是否存在安全问题' -f config.yaml -f .env.example", workdir="~/project")
```

使用 `--thinking` 显示模型思考过程：

```
terminal(command="opencode run '调试为什么测试在CI中失败' --thinking", workdir="~/project")
```
...
**重要提示：** 不要使用 `/exit`——它不是有效的 OpenCode 命令，会打开一个代理（Agent）选择对话框。请使用 Ctrl+C（`\x03`）或 `process(action="kill")` 退出。
...
| `--session <id>` / `-s` | 继续指定会话 |
| `--agent <name>` | 选择 OpenCode 代理（Agent）（构建或计划） |
...
- `/exit` 不是有效命令——它会打开代理（Agent）选择器。请使用 Ctrl+C 退出 TUI。
- PATH 不匹配可能导致选择了错误的 OpenCode 二进制/模型配置。
- 如果 OpenCode 看起来卡住了，在杀死之前先检查日志：
  - `process(action="log", session_id="<id>")`
- 避免在并行 OpenCode 会话之间共享同一工作目录。
- 在 TUI 中可能需要按两次 Enter 才能提交（一次完成文本，一次发送）。
...
```
terminal(command="opencode run '精准回复：OPENCODE_SMOKE_OK'")
```
...
1. 优先使用 `opencode run` 进行一次性自动化——更简单且不需要 pty。
2. 仅在需要迭代时使用交互式后台模式。
3. 始终将 OpenCode 会话限定在单个仓库/工作目录内。
4. 对于长时间任务，通过 `process` 日志提供进度更新。
5. 报告具体成果（更改的文件、测试、剩余风险）。
