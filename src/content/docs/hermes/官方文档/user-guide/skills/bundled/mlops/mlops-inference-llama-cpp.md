...
用于本地区 GGUF 推理及 HF Hub 模型发现的 llama.cpp。
...
|---|---|
| 来源（Source） | 内置（默认安装） |
| 路径（Path） | `skills/mlops/inference/llama-cpp` |
| 版本（Version） | `2.1.2` |
| 作者（Author） | Orchestra Research |
| 许可证（License） | MIT |
| 依赖（Dependencies） | `llama-cpp-python>=0.2.0` |
| 平台（Platforms） | linux, macos, windows |
| 标签（Tags） | `llama.cpp`, `GGUF`, `Quantization`, `Hugging Face Hub`, `CPU Inference`, `Apple Silicon`, `Edge Deployment`, `AMD GPUs`, `Intel GPUs`, `NVIDIA`, `URL-first` |

## 参考：完整 SKILL.md

:::info
以下为 Hermes 在触发该技能时加载的完整技能定义。这是技能激活时，代理（Agent）所见到的指令。
:::

# llama.cpp + GGUF

使用此技能进行本地 GGUF 推理、量化选择或面向 llama.cpp 的 Hugging Face 仓库发现。
...
- 根据用户的内存（RAM/VRAM）选择 Q4/Q5/Q6/IQ 变体
...
3. 当本地应用片段可见时，将其视为真实来源：
   ...
   - 按照 HF 显示的方式报告推荐的量化配置
4. 读取相同的 `?local-app=llama.cpp` URL 页面文本或 HTML，提取“硬件兼容性（Hardware compatibility）”下的部分：
   - 优先使用其精确的量化标签和大小，而非通用表格
   - 保留仓库特定的标签，如 `UD-Q4_K_M` 或 `IQ4_NL_XL`
   - 如果该部分在获取的页面源码中不可见，则说明情况并回退到 tree API 加通用量化指导
5. 查询 tree API 以确认实际存在的文件：
   - `https://huggingface.co/api/models/<repo>/tree/main?recursive=true`
   - 保留 `type` 为 `file` 且 `path` 以 `.gguf` 结尾的条目
   - 使用 `path` 和 `size` 作为文件名和字节大小的真实来源
   - 将量化检查点与 `mmproj-*.gguf` 投影器文件及 `BF16/` 分片文件分开
   - 仅将 `https://huggingface.co/<repo>/tree/main` 作为人工回退
6. 如果本地应用片段无法通过文本获取，则根据仓库和所选量化重建命令：
...
当 tree API 显示自定义文件命名或缺少精确的 HF 片段时使用此方法。
...
## Python 绑定（Python bindings）（llama-cpp-python）
...
## 选择量化配置（Choosing a quant）

优先使用 Hub 页面，其次是通用启发式方法。

- 优先选择 HF 标记为与用户硬件配置兼容的精确量化。
- 对于通用对话，从 `Q4_K_M` 开始。
- 对于代码或技术工作，如果内存允许，优先选 `Q5_K_M` 或 `Q6_K`。
- 对于内存预算非常紧张的情况，考虑 `Q3_K_M`、`IQ` 变体或 `Q2` 变体，但仅在用户明确优先考虑适配而非质量时使用。
- 对于多模态仓库，单独提及 `mmproj-*.gguf`。投影器不是主模型文件。
- 不要标准化仓库原生的标签。如果页面显示 `UD-Q4_K_M`，则报告 `UD-Q4_K_M`。

## 从仓库提取可用的 GGUF（Extracting available GGUFs from a repo）
...
对于像 `unsloth/Qwen3.6-35B-A3B-GGUF` 这样的仓库，本地应用页面可能显示诸如 `UD-Q4_K_M`、`UD-Q5_K_M`、`UD-Q6_K` 和 `Q8_0` 的量化芯片，而 tree API 则暴露精确的文件路径，如 `Qwen3.6-35B-A3B-UD-Q4_K_M.gguf` 和 `Qwen3.6-35B-A3B-Q8_0.gguf` 及其字节大小。使用 tree API 将量化标签转换为精确的文件名。
...
- **[hub-discovery.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/mlops/inference/llama-cpp/references/hub-discovery.md)** - 纯 URL 的 Hugging Face 工作流、搜索模式、GGUF 提取和命令重建
- **[advanced-usage.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/mlops/inference/llama-cpp/references/advanced-usage.md)** — 推测解码（speculative decoding）、批量推理（batched inference）、语法约束生成（grammar-constrained generation）、LoRA、多 GPU、自定义构建、基准测试脚本
...
- **Hugging Face 本地应用（Local Apps）文档**: https://huggingface.co/docs/hub/main/local-apps
- **Hugging Face 本地代理（Local Agents）文档**: https://huggingface.co/docs/hub/agents-local
