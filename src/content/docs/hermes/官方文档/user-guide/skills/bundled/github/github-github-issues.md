...
:::info
以下是 Hermes 在触发此技能时加载的完整技能定义。这是当技能激活时代理所看到的指令。
:::
...
- 位于包含 GitHub 远程仓库的 git 仓库中，或显式指定仓库
...
3. Log in
4. Actual: redirected to /dashboard (应该去 /settings)
...
当 PR 合并时，如果正文中包含以下关键词，issues 会被自动关闭：
...
2. **逐个阅读并分类**每个 issue（查看详情，理解 bug 或功能）

3. **应用标签和优先级**（参见上方管理 Issues 部分）
