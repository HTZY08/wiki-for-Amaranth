...
使用 `memo` 直接从终端管理 Apple Notes。笔记通过 iCloud 在 Apple 设备间同步。

## 前提条件

- 安装了 Notes.app 的 **macOS**
- 安装：`brew tap antoniorodr/memo && brew install antoniorodr/memo/memo`
- 在提示时授予对 Notes.app 的自动化访问权限（系统设置 → 隐私与安全性 → 自动化）
...
- Obsidian 保管库管理 → 使用 `obsidian` 技能
- Bear Notes → 独立应用（此处不支持）
- 仅限代理（Agent）使用的快速笔记 → 改用 `memory` 工具
...
- 无法编辑包含图片或附件的笔记
- 交互式提示需要终端访问权限（如有需要，使用 pty=true）
- 仅限 macOS — 需要 Apple Notes.app

## 规则

1. 当用户需要跨设备同步（iPhone/iPad/Mac）时，优先使用 Apple Notes
2. 对于无需同步的代理（Agent）内部笔记，使用 `memory` 工具
3. 对于基于 Markdown 的知识管理，使用 `obsidian` 技能