...
使用 Scrapling 进行网页抓取 - HTTP 获取、隐身浏览器自动化、绕过 Cloudflare、通过 CLI 和 Python 进行爬虫抓取。
...
| 标签 | `网页抓取（Web Scraping）`, `浏览器（Browser）`, `Cloudflare`, `隐身（Stealth）`, `爬虫（Crawling）`, `蜘蛛（Spider）` |
...
:::info
以下是该技能被触发时 Hermes 加载的完整技能定义。这是技能激活后代理（Agent）看到的指令。
:::

# Scrapling

[Scrapling](https://github.com/D4Vinci/Scrapling) 是一个网页抓取框架，具有反机器人绕过、隐身浏览器自动化和爬虫框架。它提供三种获取策略（HTTP、动态 JS、隐身/Cloudflare）以及完整的 CLI。
...
| 动态（Dynamic） | `DynamicFetcher` / `DynamicSession` | JS 渲染内容、SPA |
| 隐身（Stealth） | `StealthyFetcher` / `StealthySession` | Cloudflare、反机器人保护站点 |
| 爬虫（Spider） | `Spider` | 多页面爬取并跟踪链接 |
...
适用于需要 JavaScript 执行的页面（SPA、懒加载内容）：
...
所有获取器（Fetcher）都返回一个 `Selector` 对象，支持以下方法：
...
page.find_by_regex(r'\$\d+\.\d{2}')       # 按正则表达式匹配
```
...
- **合法性**：抓取前始终检查 robots.txt 和网站服务条款。本库仅用于教育和研究目的
