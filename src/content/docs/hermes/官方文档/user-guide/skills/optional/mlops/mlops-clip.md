...
OpenAI 连接视觉与语言的模型。支持零样本图像分类、图像-文本匹配和跨模态检索。基于 4 亿图像-文本对训练而成。适用于图像搜索、内容审核或无需微调的视觉-语言任务。最适合通用图像理解。
