...
通过 inference.sh CLI (infsh) 运行 150+ 个 AI 应用——图像生成、视频创作、LLM（大语言模型）、搜索、3D、社交自动化。使用终端工具。触发词：inference.sh, infsh, ai apps, flux, veo, image generation, video generation, seedream, seedance, tavily
...
:::info
以下为 Hermes 在该技能触发时加载的完整技能定义。这是技能激活时代理（Agent）看到的指令。
:::
...
完整设置步骤请参见 `references/authentication.md`。

## 工作流程

### 1. 始终先搜索

切勿猜测应用名称——始终先搜索以找到正确的应用 ID：
...
使用搜索结果的精确应用 ID。始终使用 `--json` 以获得机器可读的输出：
...
JSON 输出包含生成媒体的 URL。使用 `MEDIA:<url>` 格式向用户内联展示。
...
1. **切勿猜测应用 ID**——始终先运行 `infsh app list --search <term>`。应用 ID 会变化，且新应用频繁添加。
2. **始终使用 `--json`**——原始输出难以解析。`--json` 标志提供包含 URL 的结构化输出。
3. **检查身份验证**——如果命令因身份验证错误失败，请运行 `infsh login` 或验证是否设置了 `INFSH_API_KEY`。
4. **长时间运行的应用**——视频生成可能需要 30-120 秒。终端工具超时时间应足够，但请提醒用户可能需要等待片刻。
