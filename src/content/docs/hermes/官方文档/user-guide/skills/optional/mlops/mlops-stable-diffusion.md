...
# 稳定扩散图像生成（Stable Diffusion Image Generation）

通过 HuggingFace Diffusers 使用稳定扩散（Stable Diffusion）模型进行最先进的文本到图像生成。在需要根据文本提示生成图像、执行图像到图像转换、修复（inpainting）或构建自定义扩散管道时使用。
...
| 许可证 | MIT |
| 依赖 | `diffusers>=0.30.0`, `transformers>=4.41.0`, `accelerate>=0.31.0`, `torch>=2.0.0` |
...
:::info
以下是 Hermes 在触发此技能时加载的完整技能定义。当技能激活时，代理会将其视为指令。
:::
...
## 何时使用稳定扩散（Stable Diffusion）

**以下情况使用稳定扩散：**
- 根据文本描述生成图像
- 执行图像到图像转换（风格迁移、增强）
- 修复（inpainting）（填充遮罩区域）
- 外绘（outpainting）（将图像扩展到边界之外）
- 创建现有图像的变体
- 构建自定义图像生成工作流

**主要特性：**
- **文本到图像（Text-to-Image）**：根据自然语言提示生成图像
- **图像到图像（Image-to-Image）**：通过文本指导转换现有图像
- **修复（Inpainting）**：用上下文感知内容填充遮罩区域
- **ControlNet**：添加空间条件（边缘、姿势、深度）
- **LoRA 支持**：高效微调和风格适配
- **多模型支持**：SD 1.5, SDXL, SD 3.0, Flux

**替代方案：**
- **DALL-E 3**：基于 API 的生成（无需 GPU）
...
| `StableDiffusionInpaintPipeline` | 修复 (Inpainting) |
