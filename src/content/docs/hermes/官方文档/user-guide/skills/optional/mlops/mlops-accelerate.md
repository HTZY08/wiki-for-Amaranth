...
:::info
以下是当此技能被触发时 Hermes 加载的完整技能定义。这是 agent（代理）在技能激活时所看到的指令。
:::
...
**Megatron 集成**：请参阅 [references/megatron-integration.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/accelerate/references/megatron-integration.md) 以了解张量并行、流水线并行和序列并行的设置。

**自定义插件**：请参阅 [references/custom-plugins.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/accelerate/references/custom-plugins.md) 以了解如何创建自定义分布式插件和高级配置。

**性能调优**：请参阅 [references/performance.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/accelerate/references/performance.md) 以了解性能分析、内存优化和最佳实践。
...
- 被以下项目使用：HuggingFace Transformers、TRL、PEFT、所有 HF 库