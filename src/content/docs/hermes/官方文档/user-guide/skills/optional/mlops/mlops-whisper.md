...
OpenAI 的通用语音识别模型（General-purpose Speech Recognition Model）。支持 99 种语言、转录、翻译成英语以及语言识别。六种模型规模，从 tiny（39M 参数）到 large（1550M 参数）。用于语音转文本（Speech-to-Text）、播客转录或多语言音频处理（Multilingual Audio Processing）。最适合稳健的多语言 ASR。
...
:::info
以下是当该技能被触发时 Hermes 加载的完整技能定义。这就是技能激活时代理所看到的指令。
:::

# Whisper - 稳健的语音识别（Robust Speech Recognition）

OpenAI 的多语言语音识别模型（Multilingual Speech Recognition Model）。
...
**替代方案**：
- **AssemblyAI**：托管 API，说话人分离（Speaker Diarization）
- **Deepgram**：实时流式 ASR
...
# 支持的语言：en, es, fr, de, it, pt, ru, ja, ko, zh 等 89 种
```
...
1. **幻觉（Hallucinations）** - 可能重复或编造文本
2. **长段准确性** - 在超过 30 分钟的音频上性能下降
3. **说话人识别** - 不支持说话人分离（Diarization）
4. **口音** - 质量因口音而异
5. **背景噪声** - 可能影响准确性
6. **实时延迟** - 不适合实时字幕
