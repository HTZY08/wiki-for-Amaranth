...
使用 Flash Attention 优化 Transformer 注意力，实现 2-4 倍加速和 10-20 倍内存缩减。适用于训练/运行带有长序列（>512 tokens）的 Transformer、遇到注意力 GPU 内存问题，或需要更快推理的场景。支持 PyTorch 原生 SDPA、flash-attn 库、H100 FP8 以及滑动窗口注意力。
...
:::info
以下为 Hermes 触发此技能时加载的完整技能定义。这是技能激活时代理所看到的指令。
:::
...
Flash Attention 通过 IO 感知的平铺（IO-aware tiling）和重新计算（recomputation），为 Transformer 注意力提供 2-4 倍加速和 10-20 倍内存缩减。
...
预期：对于超过 512 tokens 的序列，加速 2-4 倍。
...
### 工作流 2：使用 flash-attn 库的高级特性

用于多查询注意力（Multi-query Attention）、滑动窗口或 H100 FP8。
...
```
FP8 配置：
- [ ] 第 1 步：确认 H100 GPU 可用
- [ ] 第 2 步：安装支持 FP8 的 flash-attn
- [ ] 第 3 步：将输入转换为 FP8
- [ ] 第 4 步：运行 FP8 注意力
```
...
- 训练序列长度大于 512 tokens 的 Transformer
- 运行长上下文推理（>2K tokens）
...
**改用替代方案的场景：**
- **标准注意力**：序列长度 <256 tokens（开销不划算）
- **xFormers**：需要更多注意力变体（不仅是速度）
- **内存高效注意力（Memory-efficient attention）**：CPU 推理（Flash Attention 需要 GPU）
...
Flash Attention 的收益随序列长度增加而增大：
- <512 tokens：加速效果极小（10-20%）
- 512-2K tokens：2-3 倍加速
- >2K tokens：3-4 倍加速
...
**与 HuggingFace Transformers 的集成**：参见 [references/transformers-integration.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/flash-attention/references/transformers-integration.md) 以了解如何在 BERT、GPT、Llama 模型中启用 Flash Attention。

**性能基准测试**：参见 [references/benchmarks.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/flash-attention/references/benchmarks.md) 以获取跨 GPU 和序列长度的详细速度与内存对比。

## 硬件要求

- **GPU**：NVIDIA Ampere+（A100, A10, A30）或 AMD MI200+
- **VRAM**：与标准注意力相同（Flash Attention 不会增加内存）
- **CUDA**：12.0+（最低 11.8）
- **PyTorch**：2.2+ 以获得原生支持
