...
用于LLM对齐的简单偏好优化。作为DPO的无参考替代方案，性能更好（在AlpacaEval 2.0上提升6.4个百分点）。无需参考模型，比DPO更高效。当需要比DPO/PPO更简单、更快速的训练时，用于偏好对齐。
...
:::info
以下是 Hermes 在触发此技能时加载的完整技能定义。这是技能激活时，智能体（agent）所看到的指令。
:::
...
SimPO 是一种无参考的偏好优化方法，无需参考模型即可超越 DPO。
...
**损失函数**：参见 [references/loss-functions.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/simpo/references/loss-functions.md) 了解 sigmoid 与 hinge 损失、数学公式以及何时使用每种方法。

**超参数调优**：参见 [references/hyperparameters.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/simpo/references/hyperparameters.md) 获取 beta、gamma、学习率选择指南以及针对不同模型大小的推荐。
...
- Alignment 手册：https://github.com/huggingface/alignment-handbook