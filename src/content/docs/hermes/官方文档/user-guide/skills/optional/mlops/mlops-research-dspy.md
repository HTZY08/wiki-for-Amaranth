...
DSPy：声明式语言模型编程，自动优化提示，RAG。

## 技能元数据（Skill metadata）

| | |
|---|---|
| 来源（Source） | 可选（Optional） — 使用 `hermes skills install official/mlops/dspy` 安装 |
| 路径（Path） | `optional-skills/mlops/research/dspy` |
| 版本（Version） | `1.0.0` |
| 作者（Author） | Orchestra Research |
| 许可证（License） | MIT |
| 依赖（Dependencies） | `dspy`, `openai`, `anthropic` |
| 平台（Platforms） | linux, macos, windows |
| 标签（Tags） | `提示工程（Prompt Engineering）`, `DSPy`, `声明式编程（Declarative Programming）`, `RAG`, `代理（Agents）`, `提示优化（Prompt Optimization）`, `语言模型编程（LM Programming）`, `斯坦福自然语言处理（Stanford NLP）`, `自动优化（Automatic Optimization）`, `模块化人工智能（Modular AI）` |
...
**GitHub 星标数**：22,000+ | **创建者**：斯坦福自然语言处理（Stanford NLP）
...
response = cot(problem="如果 John 有5个苹果，给 Mary 2个，他还剩几个？")
...
#### MIPRO（最重要提示优化，Most Important Prompt Optimization）
迭代改进提示：
...
result = extractor(text="John Doe 是一位35岁的软件工程师。")
...
|------|---------|-----------|------|
| 提示工程（Prompt Engineering） | 手动 | 手动 | 自动 |
| 优化（Optimization） | 试错 | 无 | 数据驱动 |
| 模块化（Modularity） | 低 | 中 | 高 |
| 类型安全（Type Safety） | 否 | 有限 | 是（签名） |
| 可移植性（Portability） | 低 | 中 | 高 |
| 学习曲线（Learning Curve） | 低 | 中 | 中-高 |
