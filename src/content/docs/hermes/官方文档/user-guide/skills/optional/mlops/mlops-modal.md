...
--- body ---
{/* 此页面由 website/scripts/generate-skill-docs.py 根据技能（Skill）的 SKILL.md 自动生成。请编辑 SKILL.md 源文件，而非此页面。 */}

# Modal Serverless Gpu

用于运行机器学习（ML）工作负载的无服务器 GPU 云平台。当您需要按需访问 GPU 而无需管理基础设施、将 ML 模型部署为 API，或运行具有自动扩展功能的批处理作业时，可使用此平台。
...
:::info
以下是该技能（Skill）被触发时 Hermes 加载的完整技能定义。这是代理（Agent）在技能活动时看到的指令。
:::
...
**在以下情况下使用 Modal：**
- 运行 GPU 密集型 ML 工作负载，而无需管理基础设施
- 将 ML 模型部署为自动扩展的 API
- 运行批处理作业（训练、推理、数据处理）
- 需要按秒付费的 GPU 定价，且无需空闲成本
- 快速原型化 ML 应用程序
- 运行定时任务（类 cron 的工作负载）
...
- **自动扩展**：可缩放到零，也可瞬间扩展到 100+ GPU
- **亚秒级冷启动**：基于 Rust 的基础设施，实现快速容器启动
- **容器缓存**：缓存镜像层，支持快速迭代
- **Web 端点**：将函数部署为 REST API，且支持零停机更新

**替代方案：**
- **RunPod**：用于需要持久状态的长时间运行 Pod
...
| `Cls` | 基于类的函数，带有生命周期钩子（Lifecycle Hooks） |
| `Image` | 容器镜像定义 |
...
| `H100` | 80GB | 最快，支持 FP8 + Transformer 引擎 |
...
```python
@app.function(
    container_idle_timeout=300,  # 保持热状态 5 分钟
...
@app.function(
    ...
    container_idle_timeout=120,# 保持热状态 2 分钟
    retries=3,                 # 失败时重试
...
| GPU 内存不足（OOM） | 使用更大的 GPU（`A100-80GB`），启用梯度检查点（Gradient Checkpointing） |
| 镜像构建失败 | 固定依赖版本，检查 CUDA 兼容性 |
| 超时错误 | 增加 `timeout`，添加检查点（Checkpointing） |
